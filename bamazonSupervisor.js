...
function viewProducts(option) {
    ...
    let query = `SELECT departments.department_id, departments.department_name, departments.over_head_costs, IFNULL(SUM(products.product_sales),0) AS product_sales, (IFNULL(SUM(products.product_sales),0)-departments.over_head_costs) AS total_profit
    FROM products
